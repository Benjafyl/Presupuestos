...
CMD ["sh", "-c", "test -n \"$DATABASE_URL\" || (echo 'DATABASE_URL no configurada' && exit 1); npx prisma migrate deploy && node scripts/init-db.mjs && npm run start -- --hostname 0.0.0.0 --port ${PORT:-3000}"]
